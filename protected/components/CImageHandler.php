...
class CImageHandler extends CApplicationComponent
{
    ...
    public function resize($toWidth, $toHeight, $proportional = true)
    {
        ...
        if ($proportional)
        {
            ...
            {
                $toHeight = round($toWidth / $this->width * $this->height);
            }
            else
            {
                $toWidth = round($toHeight / $this->height * $this->width);
            }
        }
    ...
    public function resizeInRect($toWidth, $toHeight, $color = array(255, 255, 255))
    {
        $this->checkLoaded();
        ...
        {
            $calcWidth = $toHeight * $ratio;
        }
        else
        {
            $calcHeight = $toWidth / $ratio;
        }

        $destX = ($toWidth - $calcWidth) / 2;
        $destY = ($toHeight - $calcHeight) / 2;

        $newImage = imagecreatetruecolor($toWidth, $toHeight);

        $this->preserveTransparency($newImage);
        
        $backgroundColor = imagecolorallocate($newImage, $color[0], $color[1], $color[2]);
        imagefill($newImage, 0, 0, $backgroundColor);

        imagecopyresampled($newImage, $this->image, $destX, $destY, 0, 0, $calcWidth, $calcHeight, $this->width, $this->height);
	...
	public function adaptiveThumb($width, $height)
	{
		...
		$this->crop($width, $height, false, false);

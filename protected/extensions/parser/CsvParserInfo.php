...
class CsvParserInfo
{
    ...
    //Диски 6.5J16 ET52.5  D67.1 CATWILD SA1  (5x114.3) GMF
    //Диски 6.0J15 ET52.5  D63.3 TREBL FORD  (5x108) Black арт.YA725
    ...
    //Диски 5J13 ET35  D58 СКАД СПИРИТ (SPIRIT)  (4x98) Белый арт.1310023
    public function parsedisc3($string, $type)
	{
		if(preg_match('!^Диски\pZ+(\d+)J(\d+)\pZ+ET(\d+)\pZ+D(\d+)\pZ+(.*\pZ+\(.*?\))\pZ+\((.*?)\)\pZ+(.*)$!isUu', $string, $result) )
		{
			return array(
				'type' => $type,
				'd' => $result[2],
    ...
    //Диски 5.5J13 ET35  D58.6 СКАД СПИРИТ (SPIRIT)  (4x98) Белый арт.1310023
    public function parsedisc4($string, $type)
	{
		if(preg_match('!^Диски\pZ+(\d+\.\d+)J(\d+)\pZ+ET(\d+)\pZ+D(\d+\.\d+)\pZ+(.*\pZ+\(.*?\))\pZ+\((.*?)\)\pZ+(.*)$!isUu', $string, $result) )
		{
			return array(
				'type' => $type,
				'd' => $result[2],
    ...
    //Диски 5.5J13.2 ET35.2  D58.6 СКАД СПИРИТ (SPIRIT)  (4x98) Белый арт.1310023
    public function parsedisc5($string, $type)
	{
		if(preg_match('!^Диски\pZ+(\d+\.\d+)J(\d+\.\d+)\pZ+ET(\d+\.\d+)\pZ+D(\d+\.\d+)\pZ+(.*\pZ+\(.*?\))\pZ+\((.*?)\)\pZ+(.*)$!isUu', $string, $result) )
		{
			return array(
				'type' => $type,
				'd' => $result[2],
     ...
     //Диски 8.0J16 ET-15  D108.5 SLIK L79  (5x139.7) S2
    ...
    //Диски 8.0J16 ET-11  УАЗ  (5x139.7) Black Suzuki
    public function parsedisc13($string, $type)
	{
		if(preg_match('!^Диски\pZ+(\d+\.\d+)\pL+(\d+)\pZ+ET\-(\d+)\pZ+(.*)\pZ+\((.*?)\)\pZ+(.*)$!isUu', $string, $result) )
		{
			return array(
				'type' => $type,
				'd' => $result[2],
    ...
    //Диски 10.0J15 ET-10  D58.6 КиК SPORT CAR  (4x98) Алмаз
    public function parsedisc14($string, $type)
	{
		if(preg_match('!^Диски\pZ+(\d+\.\d+)\pL+(\d+)\pZ+ET\pZ(\d+)\pZ+D(\d+\.\d+)\pZ+(.*)\pZ+\((.*?)\)\pZ+(.*)$!isUu', $string, $result) )
		{
			return array(
				'type' => $type,
				'd' => $result[2],
    ...
    //Диски 8.5x20 ET163  D281 LEMMERZ / ТЯГАЧ  (10x335) (208596/2700583)
    public function parsedisc15($string, $type)
	{
		if(preg_match('!^Диски\pZ+(\d+\.\d+)\pL+(\d+)\pZ+ET(\d+)\pZ+D(\d+)\pZ+(.*)\pZ+\((.*?)\)\pZ+(.*)$!isUu', $string, $result) )
		{
			return array(
				'type' => $type,
				'd' => $result[2],
    ...
    //Диски 11.75x19.5 ET0 D221 LEMMERZ / ПРИЦЕП  (8x275)
        public function parsedisc16($string, $type)
	{
		if(preg_match('!^Диски\pZ+(\d+\.\d+)\pL+(\d+\.\d+)\pZ+ET(\d+)\pZ+D(\d+)\pZ+(.*)\pZ+\((.*?)\)$!isUu', $string, $result) )
		{
			return array(
				'type' => $type,
				'd' => $result[2],
    ...
    //Диски 8.0J18 ET40 OZ SARDEGNA (5x120)
    //Диски 5.5J14 ET49  АЛЬФА  (4x100)
    public function parsedisc25($string, $type)
	{
		if(preg_match('!^Диски\pZ+(\d+\.\d+)J(\d+)\pZ+ET(\d+)\pZ+(.*)\pZ+\((.*?)\)$!isUu', $string, $result) )
		{
			return array(
				'type' => $type,
				'd' => $result[2],
    ...
    //Диски 6.75x17.5 ET0 D161 LEMMERZ / ПРИЦЕП  (6x205) арт.2870063
    public function parsedisc21($string, $type)
	{
		if(preg_match('!^Диски\pZ+(\d+\.\d+)\pL+(\d+\.\d+)\pZ+ET(\d+)\pZ+D(\d+)\pZ+(.*)\pZ+\((.*?)\)\pZ+(.*)$!isUu', $string, $result) )
		{
			return array(
				'type' => $type,
				'd' => $result[2],
	...
	{
		if(preg_match('!^Диски\pZ+(\d+\.\d+)\pL+(\d+)\pZ+ET(\d+)\pZ+(.*)$!isUu', $string, $result) )
		{
			return array(
				'type' => $type,
				'd' => $result[2],
    ...
    //Диски 6.00X17.5 ET115  Валдай НИЖНИЙ
        public function parsedisc24($string, $type)
	{
        ...
        public function parsedisc26($string, $type)
	{
    ...
    //Диски 6.0J15 ET52.5  D63.3 MEFRO FORD FOCUS
    public function parsedisc27($string, $type)
	{
		if(preg_match('!^Диски\pZ+(\d+\.\d+)\pL+(\d+)\pZ+ET(\d+\.\d+)\pZ+D(\d+)\pZ+(.*)$!isUu', $string, $result) )
		{
			return array(
				'type' => $type,
				'd' => $result[2],
        ...
        public function parsedisc28($string, $type)
	{
        ...
        public function parsedisc29($string, $type)
	{
        ...
        public function parsedisc30($string, $type)
	{
        ...
        public function parsedisc31($string, $type)
	{
        ...
        public function parsedisc32($string, $type)
	{
        ...
        public function parsedisc33($string, $type)
	{
        ...
        public function parsedisc34($string, $type)
	{
